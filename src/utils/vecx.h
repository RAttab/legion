...
inline struct vecx_name *vecx_fn(append) (struct vecx_name *vec, uint64_t val)
{
...
{
    struct vecx_name *new = vecx_fn(grow) (NULL, old->cap);
    memcpy(new, old, sizeof(*old) + old->len * sizeof(old->val[0]));
    return new;
}
...
inline bool vecx_fn(eq) (const struct vecx_name *lhs, const struct vecx_name *rhs)
{
