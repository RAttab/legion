        ...
        const void *data, size_t len)
{
    ...
    if (head->magic != 0x646165487375704FUL) // little-ending encoding of OpusHead
    ...
    while (it < end) {
        if (track->it == track->end) {
            ...
            if (track->file.it == track->file.end) {
                if (loop) sound_track_reset(track);
                else { memset(&track->file, 0, sizeof(track->file)); break; }
            }

            struct sound_page page = sound_next_page(&track->file);

            constexpr int fec = true;
            int samples = opus_decode_float(
                    track->opus, page.data, page.len, track->frame, sound_frame_len, fec);
            if (samples < 0) fail_opus(samples, "opus_decode_float");
            assert(samples > 0);

            track->it = track->frame;
            track->end = track->it + sound_frame_len;
        }
...
static void sound_queue(void)
{
    ...
    for (size_t i = 0; i < array_len(sound.queue.sfx); ++i) {
        ...
        for (size_t j = 0; j < array_len(sound.tracks.sfx); ++j) {
            ...
            if (!track->file.type) sound_track_set(track, sound.files + sfx);
        }
    }
}
