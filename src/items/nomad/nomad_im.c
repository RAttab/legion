...
static word_t im_nomad_encode_cargo(struct im_nomad *nomad, size_t ix)
{
    word_t word = 0;
...
static void im_nomad_decode_cargo(struct im_nomad *nomad, size_t ix, word_t word)
{
    assert(ix * im_nomad_data_cargo < array_len(nomad->cargo));
...
static const word_t im_nomad_io_list[] =
{
    IO_PING,
    IO_STATE,
    IO_RESET,
