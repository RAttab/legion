...
enum legion_packed item
{
    ...
    ITEM_ROD    = ITEM_PASSIVE_FIRST + 0x05,
    ITEM_TORUS  = ITEM_PASSIVE_FIRST + 0x06,
    ITEM_LIMB   = ITEM_PASSIVE_FIRST + 0x08, // assembly
    ITEM_SPINAL = ITEM_PASSIVE_FIRST + 0x09,

    // T0 - boot-legion
    ITEM_LENS   = ITEM_PASSIVE_FIRST + 0x10, // printer
    ITEM_NERVE  = ITEM_PASSIVE_FIRST + 0x11,
    ITEM_NEURON = ITEM_PASSIVE_FIRST + 0x12,
    ITEM_RETINA = ITEM_PASSIVE_FIRST + 0x13,
    ITEM_STEM   = ITEM_PASSIVE_FIRST + 0x18, // assembly
    ITEM_LUNG   = ITEM_PASSIVE_FIRST + 0x19,
    ITEM_ENGRAM = ITEM_PASSIVE_FIRST + 0x1A,
    ITEM_CORTEX = ITEM_PASSIVE_FIRST + 0x1B,
    ITEM_EYE    = ITEM_PASSIVE_FIRST + 0x1C,

    // T1 - expand-lab
    ITEM_MAGNET        = ITEM_PASSIVE_FIRST + 0x20, // printer
    ITEM_FERROFLUID    = ITEM_PASSIVE_FIRST + 0x21,
    ITEM_SEMICONDUCTOR = ITEM_PASSIVE_FIRST + 0x22,
    ITEM_PHOTOVOLTAIC  = ITEM_PASSIVE_FIRST + 0x23, // assembly
    ITEM_FIELD         = ITEM_PASSIVE_FIRST + 0x24,
    // T1 - expand-gas
    ITEM_CONDUCTOR     = ITEM_PASSIVE_FIRST + 0x25, // printer
    ITEM_GALVANIC      = ITEM_PASSIVE_FIRST + 0x26,
    ITEM_ANTENNA       = ITEM_PASSIVE_FIRST + 0x27, // assembly

    // T2 - collider
    ITEM_BIOSTEEL      = ITEM_PASSIVE_FIRST + 0x30, // printer
    ITEM_NEUROSTEEL    = ITEM_PASSIVE_FIRST + 0x31,
    ITEM_REACTOR       = ITEM_PASSIVE_FIRST + 0x32,
    ITEM_ACCELERATOR   = ITEM_PASSIVE_FIRST + 0x33, // assembly
    ITEM_HEAT_EXCHANGE = ITEM_PASSIVE_FIRST + 0x34,
    ITEM_FURNACE       = ITEM_PASSIVE_FIRST + 0x35,
    ITEM_FREEZER       = ITEM_PASSIVE_FIRST + 0x36,
    ITEM_M_REACTOR     = ITEM_PASSIVE_FIRST + 0x37,
    ITEM_M_CONDENSER   = ITEM_PASSIVE_FIRST + 0x38,
    ITEM_M_RELEASE     = ITEM_PASSIVE_FIRST + 0x39,
    ITEM_M_LUNG        = ITEM_PASSIVE_FIRST + 0x3A,
    // T2 - nomad
    ...
    // Energy - energy produces that needs to run last as it needs to know how
    // much of it's produce was consumed.
    ITEM_FUSION = ITEM_ACTIVE_FIRST + 0x40,
