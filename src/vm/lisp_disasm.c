...
static void lisp_disasm_out(
        ...
        vm_ip ip, const char *op,
        size_t arg_len, const char *arg)
{
    disasm->line->user = ip;
...
static vm_ip lisp_disasm_ip(struct disasm *disasm)
{
...
{
    vm_ip ip = lisp_disasm_ip(disasm);
...
{
    vm_ip start = lisp_disasm_ip(disasm);
